    ...
    def update(self):
        ...
        if not (key[pygame.K_DOWN] or key[pygame.K_s]) and not (key[pygame.K_UP] or key[pygame.K_w]) and self.change_y>=0:
            ...
            for block in block_hit_list:
                ...
                """PROBLEMS TO FIX:
                -if moving l/r while jumping, will reset player to edge of platform
                -add ability to drop down from platform with down key
                """
...
def shiftCoins(coin_list,shift_x,shift_y):
    for coin in coin_list:
        coin.shift(shift_x,shift_y)
        
def shiftBullets(bullet_list,shift_x,shift_y):
    for bullet in bullet_list:
        bullet.shift(shift_x,shift_y)
...
def main():
    ...
    #save and load img_horiz to save time TEMPORARY
    #mpimg.imsave('images/desktop1_horiz.png',img_horiz)
    #img_horiz = mpimg.imread('images/desktop1_horiz.png')
    #img_horiz = rgb2gray(img_horiz)
    ...
    while not done:
        ...
        if player.rect.right >= right_thresh and current_level.world_shift_x >= DISPLAY_WIDTH - SCREEN_WIDTH:#right_thresh - SCREEN_WIDTH:
            ...
            shiftCoins(coin_list,-diff,0)
            shiftBullets(bullet_list,-diff,0)
        ...
        if player.rect.top <= up_thresh and current_level.world_shift_y <= 0:
            ...
            shiftCoins(coin_list,0,diff)
            shiftBullets(bullet_list,0,diff)
